function applyTranslations(lang) {
  ...
  elements.forEach(element => {
      ...
      // Якщо елемент має дочірні елементи з data-translate-key, пропускаємо його
      if (element.querySelector('[data-translate-key]')) return;

      // Перекладаємо текст, зберігаючи HTML-теги
      if (translations[lang] && translations[lang][key]) {
          // Якщо це елемент img, перекладаємо атрибут alt
          if (element.tagName === 'IMG') {
              element.setAttribute('alt', translations[lang][key]);
          } else {
              // Для інших елементів використовуємо innerHTML, щоб зберегти теги
