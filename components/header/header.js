...
function loadHeader() {
  ...
  xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
          ...
          // Логика подмены ссылок для второй страницы
          const currentPage = window.location.pathname;
          const mainPage = '/index.html'; // Путь к главной странице

          // Проверяем, находимся ли мы на странице TryEnglish.html
          if (currentPage.includes('TryEnglish.html')) {
              const anchorLinks = document.querySelectorAll('.header__link');
              anchorLinks.forEach(link => {
                  const anchor = link.getAttribute('href');
                  // Подменяем href, добавляя путь к главной странице
                  link.setAttribute('href', mainPage + anchor);
              });
          }
      }
  };
